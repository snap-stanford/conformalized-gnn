...
class CalibAttentionLayer(MessagePassing):
    ...
    def __init__(
        ...
        dist_to_train = dist_to_train if dist_to_train is not None else shortest_path_length(edge_index, train_mask, bfs_depth, device)
...
class GATS(nn.Module):
    def __init__(self, model, edge_index, num_nodes, train_mask, num_class, dist_to_train, heads, bias, device):
        super().__init__()
        self.model = model
        self.num_nodes = num_nodes
